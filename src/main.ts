...
function initScene(): void {
   world.cleanWorldObj();
   world.addObj(new Ball(new Vec2(state.width / 2, state.height / 4), new Vec2(-400), 20));
   world.addObj(new Ball(new Vec2(state.width / 3.4, state.height / 4), new Vec2(200), 20));
   world.addObj(new Ball(new Vec2(state.width / 2.8, state.height / 4), new Vec2(0, -400), 20));
   world.addObj(new Ball(new Vec2(state.width / 2.5, state.height / 7), new Vec2(400, 200), 20));
}
...
function handleCollisions(dt: number): void {
   ...
   for (const key in grid) {
      ...
      for (const [dx, dy] of neighborOffsets) {
         ...
         const rigidBodyArray = grid[key].filter((N2) => N2 instanceof RigidBody2D);
         const neighborRigidBodyArray = grid[neighborKey].filter((N2) => N2 instanceof RigidBody2D);
         ...
         for (let i = 0; i < rigidBodyArray.length; i++) {
            ...
            for (let j = startJ; j < neighborRigidBodyArray.length; j++) {
               ...
               if (
                  shapeA === shapeB ||
                  collisionChecked.some((pair) => pair[0] === uuids[0] && pair[1] === uuids[1])
               ) {
                  continue;
               }
...
const btnSpawnGameObject = document.getElementById('spawn-object') as HTMLButtonElement;
